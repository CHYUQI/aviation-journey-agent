...
**航司官网 > 搜索引擎。** 实测搜索引擎给的航班时刻会过时
（查到 CZ6656 "每日执飞 13:05"，而南航官网说当天没有这个航班）。
搜索引擎只能用来"找官网入口"，不能当数据源。

#### 模型只抄原文，不做翻译

抽取字段时，让模型输出**页面上的原话**，翻译成契约枚举由代码做：

```go
// 对：模型输出 "到达"，代码 mapStatusText("到达") -> departed
// 错：模型输出 "departed"，我们无法复核它有没有译错
```

#### 每个值必须回原文校验

模型抽出来的任何值，都要在页面原文里找得到才采信：

```go
if !textutil.Contains(pageText, extracted.Gate) {
    // 丢弃，字段留 null
}
```

**这是信任边界，不是优化项。** 模型会编出页面里根本不存在的数字，
而且长得跟正常结果一模一样。
...
| 问法 | 结果 |
|---|---|
| "这个航班今天延误了吗"（用搜索引擎） | ❌ 拿不到，模型会开始编 |
| "这个航班今天几点起飞、从哪到哪" | ✅ 时刻表是文本，搜得到 |
| "现在安检排队多久" | ❌ 拿不到 |
| 航司官网的航班动态页 | ✅ 有状态、登机口、实际/计划时间 |

问不到就如实返回 `null`，**不要退而求其次用别的数字顶替**。

#### 配方表表示验证进度，不是能力边界

`internal/skill/airline.go` 里的配方表，没写进去的航司含义是
**"还没验证过"**，不是"不支持"。对外措辞必须区分这两者 ——
不能把未验证说成已知结论。

#### 浏览器自动化

- 用本机已装的 Edge/Chrome + CDP，**不要引入 Playwright/Puppeteer**
- 不引入视觉模型：渲染后拿 DOM 文本就够，截图识别更贵且更难校验
- `Close()` 必须结束整个进程树（Windows 用 `taskkill /T`），
  否则 Edge 的子进程会残留
- 有的航司官网有滑块验证码（国航实测有），这类走手动兜底
